...
n.show<-16452 #length(total.pp)  # Show this many volunteers, largest to smallest
...
for(v in 1:n.show) {
   for(day in seq(1:h.max)) {
      ...
      if(is.na(n.done)) {
        n.done<-last.time[v]
        last.time[v]<-max(1/7,last.time[v]*0.9)
      }
...
pdf(file="ppdpp.pdf",
    height=33.1,width=46.8,pointsize=12)
    for(day in seq(1:h.max)) {
      for(v in seq(1:n.show)) {
          n.done<-ts[day,o[v]]
          gp<-gpar(col=rgb(1,0,0,1),fill=rgb(1,0,0,1),lwd=0.1)
	  if(is.na(n.done)) {
	     gp<-gpar(col=rgb(.9,.9,.9,1),fill=rgb(.9,.9,.9,1),lwd=0.1)
	  }
