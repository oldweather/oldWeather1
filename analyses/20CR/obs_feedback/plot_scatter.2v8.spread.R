...
obs.2<-TWCR.get.obs.1file(Year,Month,Day,Hour,version='3.3.2')
obs.2$oW.obs<-grepl('9931',obs.2$UID) # Find the oW obs
...
grid.xaxis(main=T)
grid.text('Analysis spread at ob (3.3.8)',y=unit(-3,"lines"))
grid.yaxis(,main=T)
grid.text('Analysis spread at ob (3.3.2)',x=unit(-3.5,"lines"), rot=90)
...
grid.points(x=unit(obs.8$Analysis.pressure.spread[-which(obs.8$oW.obs)],"native"),
            y=unit(obs.2$Analysis.pressure.spread[-which(obs.2$oW.obs)],"native"),
            size=unit('0.02','npc'),
            pch=20,
            gp=gp_blue)
